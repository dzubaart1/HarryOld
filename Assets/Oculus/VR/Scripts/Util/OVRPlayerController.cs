...
public class OVRPlayerController : MonoBehaviour
{
	...
	private bool isMoveBackword = false;
	private bool isReturnRight = false;
	private bool isReturnLeft = false;

	public void setMoveForward(bool isMoveForward)
	{
		this.isMoveForward = isMoveForward;
	}
	public void setMoveBackword(bool isMoveBackword)
	{
		this.isMoveBackword = isMoveBackword;
	}
	public void setReturnRight(bool isReturnRight)
	{
		this.isReturnRight = isReturnRight;
	}
	public void setReturnLeft(bool isReturnLeft)
	{
		this.isReturnLeft = isReturnLeft;
	}
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			if (OVRInput.Get(OVRInput.Button.DpadUp) || isMoveForward)
			{
				moveForward = true;
				dpad_move = true;
			}

			if (OVRInput.Get(OVRInput.Button.DpadDown) || isMoveBackword)
			{
				moveBack = true;
				dpad_move = true;
			}
		...
		if (EnableRotation)
		{
			...
			if (SnapRotation)
			{
				if (OVRInput.Get(OVRInput.Button.SecondaryThumbstickLeft) ||
					(RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.PrimaryThumbstickLeft)) || isReturnLeft)
				{
					if (ReadyToSnapTurn)
					{
						euler.y -= RotationRatchet;
					...
					(RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.PrimaryThumbstickRight)) || isReturnRight)
				{
					if (ReadyToSnapTurn)
					{
						euler.y += RotationRatchet;
			...
			else
			{
				...
				if (RotationEitherThumbstick)
				{
					...
					if (secondaryAxis.sqrMagnitude < altSecondaryAxis.sqrMagnitude || isReturnLeft || isReturnRight)
					{
						Vector2 handPoseRotation = isReturnLeft ? Vector2.left : Vector2.right;
						secondaryAxis = (isReturnLeft || isReturnRight) ? handPoseRotation : altSecondaryAxis;
					}
				}
